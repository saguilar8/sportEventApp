angular.module('myApp').controller('ListDeporteController', ['$http', '$scope', '$location', '$q', '$timeout', 'NavigationService', 'DeporteService', function($http, $scope, $location, $q, $timeout, NavigationService, DeporteService) {
	...
	$scope.add = function () {
		$location.path('/deporte/add');
	};

	$scope.edit = function (obj) {
		$location.path('/deporte/edit/' + obj._id);
	};
	$scope.view = function (obj) {
		$location.path('/deporte/' + obj._id);
	};
	$scope.delete = function (obj) {
		$location.path('/deporte/delete/' + obj._id);
	};
	...
	$scope.importData = function () {
		...
		$location.path('/import/deporte');		
	};

	$scope.exportAsCsv = function () { 
		$scope.dataReceived = false;	
		DeporteService.getFileAsCsv()
		.then(function(httpResponse) {
			sendFile(httpResponse, "deportes.csv", "text/csv");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	$scope.exportAsXml = function () { 
		$scope.dataReceived = false;	
		DeporteService.getFileAsXml().then(function(httpResponse) {
			sendFile(httpResponse, "deportes.xml", "text/xml");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	$scope.exportAsXlsx = function () {
		$scope.dataReceived = false;	
		DeporteService.getFileAsXlsx().then(function(httpResponse) {
			sendFile(httpResponse, "deportes.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	function sendFile(httpResponse, fileName, mime) {
		...
		var filename = headers["x-filename"] || fileName || "deportes.csv";

angular.module('myApp').controller('ListUsuariosController', ['$http', '$scope', '$location', '$q', '$timeout', 'NavigationService', 'UsuariosService', function($http, $scope, $location, $q, $timeout, NavigationService, UsuariosService) {
	...
	$scope.add = function () {
		$location.path('/usuarios/add');
	};

	$scope.edit = function (obj) {
		$location.path('/usuarios/edit/' + obj._id);
	};
	$scope.view = function (obj) {
		$location.path('/usuarios/' + obj._id);
	};
	$scope.delete = function (obj) {
		$location.path('/usuarios/delete/' + obj._id);
	};
	...
	$scope.deleteSelected = function() {
		...
		UsuariosService.deleteMany(ids)
		//.catch(function(err) {
		//})
		.finally(function(f) {
			$timeout(function(){
	...
	$scope.importData = function () {
		...
		$location.path('/import/usuarios');		
	};

	$scope.exportAsCsv = function () { 
		$scope.dataReceived = false;	
		UsuariosService.getFileAsCsv()
	...
	$scope.exportAsXml = function () { 
		$scope.dataReceived = false;	
		UsuariosService.getFileAsXml().then(function(httpResponse) {
	...
	$scope.exportAsXlsx = function () {
		$scope.dataReceived = false;	
		UsuariosService.getFileAsXlsx().then(function(httpResponse) {

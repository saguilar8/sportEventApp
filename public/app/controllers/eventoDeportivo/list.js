angular.module('myApp').controller('ListEventoDeportivoController', ['$http', '$scope', '$location', '$q', '$timeout', 'NavigationService', 'EventoDeportivoService', function($http, $scope, $location, $q, $timeout, NavigationService, EventoDeportivoService) {
	...
	$scope.add = function () {
		$location.path('/eventoDeportivo/add');
	};

	$scope.edit = function (obj) {
		$location.path('/eventoDeportivo/edit/' + obj._id);
	};
	$scope.view = function (obj) {
		$location.path('/eventoDeportivo/' + obj._id);
	};
	...
	$scope.deleteAll = function() {
		...
		EventoDeportivoService.deleteAll({ 
	...
	$scope.deleteSelected = function() {
		...
		EventoDeportivoService.deleteMany(ids)
		//.catch(function(err) {
		//})
		.finally(function(f) {
			$timeout(function(){
	...
	$scope.importData = function () {
		...
		$location.path('/import/eventoDeportivo');		
	};

	$scope.exportAsCsv = function () { 
		$scope.dataReceived = false;	
		EventoDeportivoService.getFileAsCsv()
		.then(function(httpResponse) {
			sendFile(httpResponse, "eventoDeportivos.csv", "text/csv");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	$scope.exportAsXml = function () { 
		$scope.dataReceived = false;	
		EventoDeportivoService.getFileAsXml().then(function(httpResponse) {
			sendFile(httpResponse, "eventoDeportivos.xml", "text/xml");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	$scope.exportAsXlsx = function () {
		$scope.dataReceived = false;	
		EventoDeportivoService.getFileAsXlsx().then(function(httpResponse) {
			sendFile(httpResponse, "eventoDeportivos.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	function sendFile(httpResponse, fileName, mime) {
		...
		var filename = headers["x-filename"] || fileName || "eventoDeportivos.csv";
	...
	$scope.loadCurrentPage = function () {
		//$scope.dataReceived = false;
		EventoDeportivoService.getList({ 
	...
	$scope.updateRecordCount = function () {
		EventoDeportivoService.getCount({ 
			'searchText' : $scope.searchText
		})
		.catch(function(err) {
			if (err) {

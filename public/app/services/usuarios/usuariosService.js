angular.module('myApp').service('UsuariosService', ['$http', '$q', 'UrlService', 'baseApi', function ($http, $q, UrlService, baseApi) {
	...
	// Getting the usuarios API documentation URL and a promise to know when it's loaded.
	docUrlReceived = UrlService.usuariosUrl.then(function (url) {
		docUrl = url;
	});

	// When the documentation is received, getting the usuarioss resource URL and a promise to know when it's loaded.
...
//Process each property
		...
		clause = addStringLike('idUsuario', searchText);

		if (clause != null) {
			clauses.push(clause);
		}

		clause = addNumberLike('cdUsuario', searchText);
		...
		clause = addStringLike('apodo', searchText);
	...
	UsuariosService.getCount =  safeCall(function (opts) {
		opts = opts || {};
		opts.count = true;		
	...
	UsuariosService.getList = safeCall(function (opts) {
		opts = opts || {};
		var q = buildBaucisQuery(opts);
		return $http.get(resourceUrl + q);
	});

	UsuariosService.getListAsCsv = safeCall(function () {
		return $http({
			method: 'GET', 
			url: resourceUrl, 
	...
	UsuariosService.getFileAsCsv = safeCall(function () {
		return $http({
			method: 'GET', 
	...
	UsuariosService.getFileAsXml = safeCall(function () {
		return $http({
			method: 'GET', 
	...
	UsuariosService.getFileAsXlsx = safeCall(function () {
		return $http({
			method: 'GET', 
	...
	UsuariosService.getToEdit = safeCall(function (id) {
		return UsuariosService.get(resourceUrl + '/' + id );
	});

	UsuariosService.get = safeCall(function (link) {
		return $http.get(link);
	});

	UsuariosService.add = safeCall(function (item) {
	...
	UsuariosService.delete = safeCall(function (id) {
		return $http.delete(resourceUrl + '/' + id);
	});

	UsuariosService.deleteMany = safeCall(function (ids) {
		var msg = { 
			'className' : 'usuarios',
			'ids'		: ids
		};	
	...
	UsuariosService.deleteAll = safeCall(function (opts) {
		var msg = { 
			'className' : 'usuarios',

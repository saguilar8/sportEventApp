angular.module('myApp').service('PerfilesService', ['$http', '$q', 'UrlService', 'baseApi', function ($http, $q, UrlService, baseApi) {
	...
	// Getting the perfiles API documentation URL and a promise to know when it's loaded.
	docUrlReceived = UrlService.perfilesUrl.then(function (url) {
		docUrl = url;
	});

	// When the documentation is received, getting the perfiless resource URL and a promise to know when it's loaded.
...
//Process each property
		...
		clause = addStringLike('idPerfil', searchText);

		if (clause != null) {
			clauses.push(clause);
		}

		clause = addNumberLike('cdPerfil', searchText);

		if (clause != null) {
			clauses.push(clause);
		}

		clause = addStringLike('dsPerfil', searchText);
	...
	PerfilesService.getCount =  safeCall(function (opts) {
		opts = opts || {};
		opts.count = true;		
	...
	PerfilesService.getList = safeCall(function (opts) {
		opts = opts || {};
		var q = buildBaucisQuery(opts);
		return $http.get(resourceUrl + q);
	});

	PerfilesService.getListAsCsv = safeCall(function () {
		return $http({
			method: 'GET', 
			url: resourceUrl, 
	...
	PerfilesService.getFileAsCsv = safeCall(function () {
		return $http({
			method: 'GET', 
	...
	PerfilesService.getFileAsXml = safeCall(function () {
		return $http({
			method: 'GET', 
	...
	PerfilesService.getFileAsXlsx = safeCall(function () {
		return $http({
			method: 'GET', 
	...
	PerfilesService.getToEdit = safeCall(function (id) {
		return PerfilesService.get(resourceUrl + '/' + id );
	});

	PerfilesService.get = safeCall(function (link) {
		return $http.get(link);
	});

	PerfilesService.add = safeCall(function (item) {
		return $http.post(resourceUrl, JSON.stringify(item));
	});

	PerfilesService.update = function (item) {
		return $http.put(resourceUrl + '/' + item._id, JSON.stringify(item));
	};

	PerfilesService.delete = safeCall(function (id) {
		return $http.delete(resourceUrl + '/' + id);
	});

	PerfilesService.deleteMany = safeCall(function (ids) {
		var msg = { 
			'className' : 'perfiles',
			'ids'		: ids
		};	
	...
	PerfilesService.deleteAll = safeCall(function (opts) {
		var msg = { 
			'className' : 'perfiles',
